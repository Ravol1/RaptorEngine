...
namespace raptor::interpreter::graphic::detail {
	...
	public:
		...
		struct LayerOptions {
			...
			std::optional<MessageLayerComponent> msg_component;
		};



		auto set_visible(bool visible, LayerPage page) {
			auto& target = get_page(page);
		...
		void swap_pages() { std::swap(fore_, back_); }


		auto create_msg_component() {
			fore_.msg_component = MessageLayerComponent{};
			back_.msg_component = MessageLayerComponent{};
		}
	...
	private:
		...
		void with_msg_component(LayerPage page, Func&& action) {
			auto& target = get_page(page);
			if (target.msg_component.has_value())
				action(*target.msg_component);
		}

		auto get_page(LayerPage page) -> LayerOptions& {
